...
/**
 * 
 * @author torniai@ohsum01.ohsu.edu TO DO:
 * 
 *         - Add parameters (out File source instance file / source ontology) -
 *         Refactor some methods - Have the ontology look up existing
 *         practitioners (future / optional parameter) - Add support for Mysql
 *         table to generate the sampleData - Create a tests with SPARQL queries
 *         with the proper result (queries are already in) - modify the
 *         clinical-instance.owl with the proper URIs and synch the URIs - Test
 *         the generated instances when we have the CPT and ICD owl files.
 * 
 * 
 *         Assumption: this is a one shot script now: it assumes to have a clean
 *         ontology with no instances. In the future we can add a target
 *         ontology where append the data -> will perform the look up for
 *         instances there rather that in the t Arraylists
 * 
 */

public class createInstances {

	public static final IRI diagnosisClassIRI = IRI.create(DIAGNOSIS_URI);
	public static final IRI hasspecifiedoutputIRI = IRI.create(HAS_SPECIFIED_OUTPUT_URI);
	public static final IRI haspartIRI = IRI.create(HAS_PART_URI);
	public static final IRI healthcarepractitionerClassIRI = IRI.create(HEALTH_PRACTITIONER_CLASS_URI);
	public static final IRI patientClassIRI = IRI.create(PATIENT_CLASS_URI);
	public static final IRI encounterCLassIRI = IRI.create(ENCOUNTER_CLASS_URI);
	public static final IRI hasdateIRI = IRI.create(HAS_DATE_DATA_URI);
	public static final IRI IdentifierAnnoIRI = IRI.create(IDENTIFIER_ANNOT_PROPERTY_URI);
	public static final IRI orderClassIRI = IRI.create(ORDER_CLASS_URI);
	public static final IRI hasparticipantIRI = IRI.create(HAS_PARTICIPANT_URI);
	public static final IRI datetimeIRI = IRI.create("http://www.w3.org/2001/XMLSchema#dateTime");
	...
	public static void main(String[] args) throws OWLOntologyCreationException, OWLOntologyStorageException {
		...
		for (int i = 0; i < testData.size(); i++) {
			...
			// Maybe the following can be extracted as a method so that it can be
			// reused
			...
			if (!practitionerIDSet.contains(practitionerID)) {
				...
				// Here we can have a function that we want to reuse for all the others
			...
			for (int j = 0; j < uniquepatients; j++) {
				...
				isCPT = false;
				isICD = false;
				// create patient instance (label practitioner_XXXXXX , annotation
				// property identifier) // Currently UUID Need to define an hash
				// function
				...
				// create an encounter instance (label label
				// encouneter_pract_XXXXX_pat_XXXXX, has_date -> value)
				...
				OWLDataPropertyAssertionAxiom dataproporaxiom = df.getOWLDataPropertyAssertionAxiom(df.getOWLDataProperty(hasdateIRI), encounter,
						df.getOWLLiteral("10-2-2012", df.getOWLDatatype(datetimeIRI)));
				manager.applyChange(new AddAxiom(onto, dataproporaxiom));

				// Add the participants in the encounter: the patientIRI and the
				// practitioner IRI
				...
				if (!CPTCode.equals("")) {
					...
					// Using http://purl.obolibrary.org/obo/OBI_0000299 for has_specified
					...
					// Process CPT Code
					// Create the CPT Instance
					// For the time being I am just creating an instance of order that has
					// part the instance of CPT code that has identifier the identifier
			...
			// If the total codes are > than the unique patients we append the codes
			// to the last encounter

			int additionalcodes = testData.get(i).codeOccurrences - uniquepatients; // Calculate
																																							// the
																																							// numbers
																																							// of
																																							// codes
